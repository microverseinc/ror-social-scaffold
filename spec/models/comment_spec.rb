RSpec.describe Comment do
  ...
  let(:comment) { Comment.create(content: 'This is content of the post', post_id: post.id, user_id: user.id) }
  let(:subject) do
    described_class.new(
      post_id: post.id,
      user_id: user.id,
  ...
  context 'with 2 or more comments' do
    it 'orders them in reverse chronologically' do
      post = Post.create!
      comment1 = post.comments.create!(body: 'first comment', user: user)
      comment2 = post.comments.create!(body: 'second comment', user: user)
