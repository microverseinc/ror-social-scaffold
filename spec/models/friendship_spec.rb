...
describe 'Friendship', type: :feature do
  describe 'Create new friendship' do
    ...
    it 'sends friend request' do
      ...
      check = user_1.friendships.build(friend_id: user_2.id, status: "pending")
      check.save
    ...
    it 'accepts friend request' do
      ...
      frie = Friendship.find_by(friend_id: user_2.id)
      expect(frie.update(user_id: user_1.id, friend_id: user_2.id, status: "approved")).to be(true)
    end
