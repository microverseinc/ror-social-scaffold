...
RSpec.describe 'Invite friend', type: :feature do
  before(:each) do
    ...
    Friendship.create(user_id: 1, friend_id: 2, confirmed: false)
  end
  ...
  it 'Verify if User accepted the invitation' do
    Friendship.update(user_id: 1, friend_id: 2, confirmed: true)
  ...
  it 'Verify if User declined the invitation' do
    friend = Friendship.find_by(user_id:1, friend_id:2)
    friend.delete
