module ApplicationHelper
  ...
  def already_sended?(user)
    if current_user.friendships.any? { |friendship| friendship.friend == user }
  ...
  def sended_to_us?(_user)
    if current_user.inverse_friendships.any? { |friendship| friendship.user == current_user }
      redirect_to root_path
  ...
  def btn_send(user)
    if current_user.friendships.none? { |friendship| friendship.friend == user }
